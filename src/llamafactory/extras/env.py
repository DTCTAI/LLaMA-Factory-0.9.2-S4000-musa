...
import peft
import torch
import torch_musa # 我加了
import transformers
import trl
from transformers.utils import is_torch_cuda_available, is_torch_npu_available, is_torch_musa_available
...
def print_env() -> None:
    ...
    if is_torch_musa_available():
        info["PyTorch version"] += " (GPU)"
        info["GPU type"] = torch.musa.get_device_name()
        info["GPU number"] = torch.musa.device_count()
        info["GPU memory"] = f"{torch.musa.mem_get_info()[1] / (1024 ** 3):.2f}GB"
    try:
